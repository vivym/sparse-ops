...
#define CASE(NDIM)                                                        \
  case NDIM: {                                                            \
    ...
    auto num_voxels = thrust_impl::voxelize<scalar_t, index_t, NDIM>(     \
        policy, voxel_coords_ptr, voxel_point_indices_ptr,                \
        voxel_indices_ptr, voxel_batch_indices_ptr,                       \
  ...
  AT_DISPATCH_FLOATING_TYPES(points.type(), "sparse_ops::voxelize::cuda::voxelize", [&] {
    if (batch_indices.has_value()) {
      voxelize_sparse<scalar_t, int64_t>(
          voxel_coords, voxel_point_indices, voxel_indices, voxel_batch_indices,
          points, batch_indices.value(), voxel_size, points_range_min, points_range_max);
    } else {
    }
  });

  return {voxel_coords, voxel_point_indices, voxel_indices, voxel_batch_indices};
}

TORCH_LIBRARY_IMPL(sparse_ops, CUDA, m) {
  m.impl(TORCH_SELECTIVE_NAME("sparse_ops::voxelize"),
         TORCH_FN(voxelize));
}
