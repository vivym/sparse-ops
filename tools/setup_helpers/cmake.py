...
USE_NINJA = (
    not check_negative_env_flag("USE_NINJA") and
...
def _overlay_windows_vcvars(env: Dict[str, str]) -> Dict[str, str]:
    ...
    vc_env: Dict[str, str] = setuptools.distutils._msvccompiler._get_vc_env(vc_arch)
...
class CMake:
    ...
    def generate(
        ...
        cmake_torch_library: Optional[str],
        rerun: bool,
    ) -> None:
        "Runs cmake to generate native build files."

        self.my_env = _create_build_env()
        ...
        for var, val in self.my_env.items():
        ...
        build_options["CMAKE_PREFIX_PATH"] = (
            cast(str, cmake_torch_library) + ";" + cast(str, py_lib_path)
        )
        if cmake_prefix_path:
            build_options["CMAKE_PREFIX_PATH"] += ";" + cast(str, cmake_prefix_path)
        ...
        if len(specified_cmake__options) > 0:
            print(", ".join(specified_cmake__options) +
                  " should not be specified in the environment variable. They are directly set by build script.")
            sys.exit(1)
        ...
        for env_var_name in self.my_env:
            if env_var_name.startswith("gh"):
                # github env vars use utf-8, on windows, non-ascii code may
                # cause problem, so encode first
                try:
                    self.my_env[env_var_name] = str(self.my_env[env_var_name].encode("utf-8"))
                except UnicodeDecodeError as e:
                    shex = ":".join("{:02x}".format(ord(c)) for c in self.my_env[env_var_name])
        ...
        self.run(args, env=self.my_env)

    def build(self) -> None:
        "Runs cmake to build binaries."

        assert self.my_env is not None
        ...
        self.run(build_args, self.my_env)
